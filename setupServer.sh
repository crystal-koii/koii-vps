...
install_koii() {
    if ! [ -x "$(command -v koii)" ]; then
        ...
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            echo "Exiting script. Please update your PATH and run step 4 again."
            exit 1
        fi
    else
    ...
    #Prompt user to copy wallet path and use finnie to fill wallet
    echo "Please take a moment to copy the wallet path information."
    echo "Please take a moment to use Finnie and fill your new wallet with enough koii to run the tasks."
}
